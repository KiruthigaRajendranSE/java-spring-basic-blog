...
public class Module2_Tests {
	...
	/*
	 * @Test public void task_8() { // Task 7 - Display list with foreach loop in
	 * template // TODO - How to more robustly test? Right now just confirming 3
	 * divs show up. Document doc = null; String errorInfo = ""; try { MvcResult
	 * result = this.mvc.perform(get("/")).andReturn(); MockHttpServletResponse
	 * response = result.getResponse(); String content =
	 * response.getContentAsString();
	 * 
	 * doc = Jsoup.parse(content); } catch (Exception e) { errorInfo =
	 * e.getLocalizedMessage(); //e.printStackTrace(); } String message =
	 * "Task 8: The template has errors - " + errorInfo + ".";
	 * assertNotNull(message, doc);
	 * 
	 * Elements h2Elements = doc.getElementsByTag("h2"); message =
	 * "Task 8: An `<h2>` tag does not exist in the home.html template.";
	 * assertTrue(message, h2Elements.size() > 0);
	 * 
	 * message = "Task 8: "+ALL_POSTS.size()
	 * +" `<h2>` tags should have been generated by the Thymeleaf each loop with an `<h2>` tag inside."
	 * ; assertTrue(message, h2Elements.size() == ALL_POSTS.size());
	 * 
	 * // Check each h2 content for (int i=0; i < h2Elements.size(); i++) { Element
	 * element = h2Elements.get(i); message =
	 * "Task 8: The `<h2>` tag displays the title \"" + element.html() +
	 * "\" instead of \"" + ALL_POSTS.get(i).getTitle() + "\".";
